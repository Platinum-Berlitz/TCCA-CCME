...
program random1d
	...
	do i = 1, ncycle
        ...
        dist(x) = dist(x) + 1
		if (i == 100) then
			write (21, *) 'Mean-Squared Displacement, Sampled from 100 Trials: '
			do k = 1, njump
				write (21, *) k, real(r2(k), 8) / (njump + 1 - k) / 100
			end do
		else if (i == 1000) then
			write (21, *) 'Mean-Squared Displacement, Sampled from 1000 Trials: '
			do k = 1, njump
				write (21, *) k, real(r2(k), 8) / (njump + 1 - k) / 1000
			end do
		end if
	end do
