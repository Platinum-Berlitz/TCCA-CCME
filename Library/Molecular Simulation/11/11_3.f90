program mcnpt ! npt simulation of hard-spheres
    ! gfortran 11_3.f90 -o 11_3 && ./11_3
	implicit none
	integer, parameter :: nstep = 2e5, ninit = 1e4, npart = 400
    integer :: i, j, k, c, q, p, count, ipart
	logical lready
	real*8 :: x(npart), y(npart), z(npart), xold(npart), yold(npart), zold(npart)
	real*8 :: r2, att1, att2, xn, yn, zn, dvol, dispvol, dispmax = 1d0, avv1, avv2, press = 1d0
    real*8 :: vnew, vold, iboxold, boxold, box = 1d1, ibox = 1d-1, dx, dy, dz, mvo1, mvo2, r1
	real*8, parameter :: dvols(10) = (/(1d-3*i, i=1, 10)/)
	real*8, parameter :: dispvols(10) = (/(i, i=1, 10)/)

    call random_seed()
	open (10, file = '11_3.dat')
	do p = 1, 2
	if (p == 1) write (10, *) 'Random Walk in ln V'
	if (p == 2) write (10, *) 'Random Walk in V'
	do q = 1, 10
		dvol = dvols(q)
		dispvol = dispvols(q)
		! initialize 
		...
		do c = 1, nstep
			...
			if (ipart == npart + 1) then
				...
				lready = .false.
				call random_number(r1)
				if (p == 1) vnew = exp(log(vold)+(r1-0.5d0)*dvol)
				if (p == 2) vnew = vold + (r1-5d-1)*dispvol
				boxold = box
				iboxold = ibox
		...
		if (p == 1) write (10, *) dvol, avv2/avv1
		if (p == 2) write (10, *) dispvol, avv2/avv1
