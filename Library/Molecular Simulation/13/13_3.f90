...
integer im
real*8, parameter :: temps(10) = (/(1d-2*im, im=1, 10)/)
...
!		 t0max = maximum number of time origins
    ...
    if (switch==1) then
        ...
        do i = 1, tmax - 1
            ...
            ! if (i/=1) then
            !     write (12, *) dtime, r2t(i), (r2t(i) - r2t(i-1)) / 2 / tstep
            ! else
            !     write (12, *) dtime, r2t(i), 0d0
            ! end if
        end do
        write (12, *) temp, (r2t(tmax-1) - r2t(tmax*9/10))/(tmax/10)/2/tstep
    end if
    return
...
program barrier1
    ! gfortran 13_3.f90 -o 13_3 && ./13_3
    use barrier_
    implicit none
    integer i, j, cyc
    real*8 cstart, cc1, cc2, tt1, tt2

    open (10, file = '13_3.dat')
    open (11, file = '13_3_trace.dat')
    open (12, file = '13_3_diff.dat')
    open (13, file = '13_3_dist.dat')
    do cyc = 1, 10
        call readdat(cyc)
        cc1 = 0.0d0
        cc2 = 0.0d0
