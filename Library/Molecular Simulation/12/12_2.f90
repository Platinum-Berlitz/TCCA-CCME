program ising ! 2d ising model
	...
	real*8 :: beta = 0.5d0, av1 = 0, av2 = 0, dist2, weight, move1 = 0, move2 = 0, dist3
    real*8 :: w(0:maxmag) = 1, dist1(-maxmag:maxmag), tstart, tend, r1, r2

    open (10, file = '12_2.dat')
	...
	do i = 1, ncycle
		do j = 1, 100*nsize**2
			...
			diff = 0d0
			move2 = move2 + 1.0d0
            ! calculate the energy difference between new and old
            ! start modification
			do k = 1, 4
				inew = ilat + iup(k)
				jnew = jlat + jup(k)
				diff = diff - (lnew - lold) * lattice(inew, jnew)
			end do
            ! end modification
	...
	dist2 = 1.0d0/dist2
	dist3 = 1.0d0/dist3
	write (10, *) 'Magnetization Distribution: '
	do i = -maxmag, maxmag, 2
		if (dist1(i)>0.5d0) then
			weight = exp(-w(abs(i)))
			write (10, *) i, dist1(i)*dist2, dist1(i)*dist3*weight
		end if
	end do
